...
import { Eye, Save, ArrowLeft, Send, FileText, X, Upload } from 'lucide-react';
import { DocumentFooter } from '../shared/DocumentFooter';

interface TrainingPreConsultFormProps {
  onBack: () => void;
  onPreview: (data: TrainingPreConsultData) => void;
  onSave?: () => void;
...
export interface TrainingPreConsultData {
  contactName: string;
  contactEmail: string;
...
export function TrainingPreConsultForm({ onBack, onPreview, onSave, backButtonLabel = "Save & Back", services, linkedService }: TrainingPreConsultFormProps) {
  ...
  const defaultTitle = 'Pre-Consultation Questionnaire: Training';
  const defaultSubtitle = 'PX Strategy Workshop & Training';
