    ...
    const saveActivities = async () => {
      ...
      try {
        ...
        if (!response.ok) {
          ...
          // Only log warning if it's not a WORKER_LIMIT error (which we now prevent with size check) and not 404/503
          if (errorData.code !== 'WORKER_LIMIT' && response.status !== 404 && response.status !== 503) {
